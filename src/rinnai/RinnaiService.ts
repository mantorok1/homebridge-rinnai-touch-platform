...
import { Command, Commands } from '../models/Command.js'
import { Fault } from '../models/Fault.js'
import { States } from '../models/Status.js'
import { RinnaiSession } from './RinnaiSession.js'
...
export class RinnaiService extends EventEmitter {
  ...
  async setOperatingMode(value: OperatingModes): Promise<void> {
    ...
    try {
      ...
      const command = new Command({ command: Commands.OperatingMode, state })
  ...
  async setOperatingState(value: OperatingStates): Promise<void> {
    ...
    try {
      ...
      const command = new Command({ command: Commands.OperatingState, state })
      await this.session.sendCommand(command)
    } catch (error) {
      if (error instanceof Error) {
        this.log.error(error.message)
  ...
  async setDayAndTime(): Promise<void> {
    ...
    try {
      ...
      const command = new Command({ command: Commands.SetDayAndTime, states })
      await this.session.sendCommand(command)
    } catch (error) {
      if (error instanceof Error) {
        this.log.error(error.message)
  ...
  async saveDayAndTime(): Promise<void> {
    ...
    try {
      const state = 'Y'
      const command = new Command({ command: Commands.SaveDayAndTime, state })
      await this.session.sendCommand(command)
    } catch (error) {
      if (error instanceof Error) {
        this.log.error(error.message)
  ...
  async setPowerState(value: boolean): Promise<void> {
    ...
    try {
      ...
      const command = new Command({ command: Commands.PowerState, state })
      await this.session.sendCommand(command)
    } catch (error) {
      if (error instanceof Error) {
        this.log.error(error.message)
  ...
  async setFanState(value: boolean): Promise<void> {
    ...
    try {
      ...
      const state = value
        ...
        : 'F'
      const command = new Command({ command: Commands.FanState, state })
      await this.session.sendCommand(command)
    } catch (error) {
      if (error instanceof Error) {
        this.log.error(error.message)
  ...
  async setFanSpeed(value: number): Promise<void> {
    ...
    try {
      ...
      const command = new Command({ command: Commands.FanSpeed, state })
      await this.session.sendCommand(command)
    } catch (error) {
      if (error instanceof Error) {
        this.log.error(error.message)
  ...
  async setSetPointTemperature(value: number, zone = 'U'): Promise<void> {
    ...
    try {
      ...
      const state = value.toString().padStart(2, '0')
      const command = new Command({ command: Commands.SetPointTemperature, zone, state })
      await this.session.sendCommand(command)
    } catch (error) {
      if (error instanceof Error) {
        this.log.error(error.message)
  ...
  async setUserEnabled(value: boolean, zone: string): Promise<void> {
    ...
    try {
      ...
      const command = new Command({ command: Commands.UserEnabled, zone, state })
      await this.session.sendCommand(command)
    } catch (error) {
      if (error instanceof Error) {
        this.log.error(error.message)
  ...
  async setControlMode(value: ControlModes, zone = 'U'): Promise<void> {
    ...
    try {
      ...
      const command = new Command({ command: Commands.ControlMode, zone, state })
      await this.session.sendCommand(command)
    } catch (error) {
      if (error instanceof Error) {
        this.log.error(error.message)
  ...
  async setScheduleOverride(value: ScheduleOverrideModes, zone = 'U'): Promise<void> {
    ...
    try {
      ...
      const command = new Command({ command: Commands.ScheduleOverride, zone, state })
      await this.session.sendCommand(command)
    } catch (error) {
      if (error instanceof Error) {
        this.log.error(error.message)
  ...
  async setPumpState(value: boolean): Promise<void> {
    ...
    try {
      const state = value ? 'N' : 'F'
      const command = new Command({ command: Commands.PumpState, state })
      await this.session.sendCommand(command)
    } catch (error) {
      if (error instanceof Error) {
        this.log.error(error.message)
