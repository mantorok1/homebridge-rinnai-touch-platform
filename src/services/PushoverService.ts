import type { Fault } from '../models/Fault.js'
...
export class PushoverService {
  ...
  private handleStatus(status: Status): void {
    ...
    const module_date = new Date(now.getFullYear(), now.getMonth(), now.getDate() + day_offset, module_hours, module_minutes, now.getSeconds())
